...
用完点「**停止模型**」结束模型进程；「**退出**」关闭本工具（模型还在跑时会先问你是否一并停止）。
...
| 状态区 | 显示当前阶段（空闲/启动中/就绪/超时/错误）；启动中会出现「跳过等待」与「停止模型」按钮 |
| 标题栏 | 可拖动；右上角最小化 / 关闭 |
| 退出 | 底部按钮，行为见下 |
...
8. **跳过等待 / 停止模型 / 退出**：
   ...
   - 停止模型：`taskkill /T /F` 整棵模型进程树。
   - 退出：模型运行中会询问「要一并停止吗？」，是 = 停模型并退出，否 = 仅退出（模型继续跑）。
...
| 「8080 已占用」 | 已有模型在跑：选「是」只开 DeepSeek Harness；或先用「停止模型」/任务管理器清理 |
